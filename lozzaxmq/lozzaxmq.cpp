...
int LozzaxMQ::set_zmq_context_option(int option, int value) {
    return context.setctxopt(option, value);
}

void LozzaxMQ::log_level(LogLevel level) {
    log_lvl.store(level, std::memory_order_relaxed);
}

LogLevel LozzaxMQ::log_level() const {
    return log_lvl.load(std::memory_order_relaxed);
}


CatHelper LozzaxMQ::add_category(std::string name, Access access_level, unsigned int reserved_threads, int max_queue) {
...
void LozzaxMQ::add_command(const std::string& category, std::string name, CommandCallback callback) {
...
void LozzaxMQ::add_request_command(const std::string& category, std::string name, CommandCallback callback) {
    add_command(category, name, std::move(callback));
    categories.at(category).commands.at(name).second = true;
}

void LozzaxMQ::add_command_alias(std::string from, std::string to) {
...
zmq::socket_t& LozzaxMQ::get_control_socket() {
    assert(proxy_thread.joinable());

    // Maps the LozzaxMQ unique ID to a local thread command socket.
    ...
    // Optimize by caching the last value; LozzaxMQ is often a singleton and in that case we're
    ...
    if (proxy_shutting_down)
        throw std::runtime_error("Unable to obtain LozzaxMQ control socket: proxy thread is shutting down");
        ...
        sn_lookup{std::move(lookup)}, log_lvl{level}, logger{std::move(logger)}
{
    ...
    LMQ_TRACE("Constructing LozzaxMQ, id=", object_id, ", this=", this);

    if (pubkey.empty() != privkey.empty()) {
        throw std::invalid_argument("LozzaxMQ construction failed: one (and only one) of pubkey/privkey is empty. Both must be specified, or both empty to generate a key.");
    } else if (pubkey.empty()) {
        if (service_node)
            throw std::invalid_argument("Cannot construct a service node mode LozzaxMQ without a keypair");
        LMQ_LOG(debug, "generating x25519 keypair for remote-only LozzaxMQ instance");
        ...
        if (verify_pubkey != pubkey)
            throw std::invalid_argument("Invalid pubkey/privkey values given to LozzaxMQ construction: pubkey verification failed");
    }
}
...
void LozzaxMQ::start() {
    ...
    LMQ_LOG(info, "Initializing LozzaxMQ ", bind.empty() ? "remote-only" : "listener", " with pubkey ", to_hex(pubkey));
    ...
    if (MAX_SOCKETS > 1 && MAX_SOCKETS <= zmq_socket_limit)
        ...
        LMQ_LOG(error, "Not applying LozzaxMQ::MAX_SOCKETS setting: ", MAX_SOCKETS, " must be in [1, ", zmq_socket_limit, "]");
    ...
    proxy_thread = std::thread{&LozzaxMQ::proxy_loop, this};
    ...
    catch (const zmq::error_t &e) { throw std::runtime_error("Failure reading from LozzaxMQ::Proxy thread: "s + e.what()); }
...
void LozzaxMQ::listen_curve(std::string bind_addr, AllowFunc allow_connection) {
...
void LozzaxMQ::listen_plain(std::string bind_addr, AllowFunc allow_connection) {
    // TODO: As above.
    check_not_started(proxy_thread, "start listening");
...
std::pair<LozzaxMQ::category*, const std::pair<LozzaxMQ::CommandCallback, bool>*> LozzaxMQ::get_command(std::string& command) {
...
void LozzaxMQ::set_batch_threads(int threads) {
    if (proxy_thread.joinable())
...
void LozzaxMQ::set_reply_threads(int threads) {
    if (proxy_thread.joinable())
...
void LozzaxMQ::set_general_threads(int threads) {
    if (proxy_thread.joinable())
...
LozzaxMQ::run_info& LozzaxMQ::run_info::load(category* cat_, std::string command_, ConnectionID conn_, Access access_, std::string remote_,
...
LozzaxMQ::run_info& LozzaxMQ::run_info::load(pending_command&& pending) {
...
LozzaxMQ::run_info& LozzaxMQ::run_info::load(batch_job&& bj, bool reply_job) {
    is_batch_job = true;
...
LozzaxMQ::~LozzaxMQ() {
    ...
    LMQ_LOG(info, "LozzaxMQ shutting down proxy thread");
    detail::send_control(get_control_socket(), "QUIT");
    proxy_thread.join();
    LMQ_LOG(info, "LozzaxMQ proxy thread has stopped");
}
