...
void LozzaxMQ::worker_thread(unsigned int index) {
...
LozzaxMQ::run_info& LozzaxMQ::get_idle_worker() {
...
void LozzaxMQ::proxy_worker_message(std::vector<zmq::message_t>& parts) {
...
void LozzaxMQ::proxy_run_worker(run_info& run) {
    if (!run.worker_thread.joinable())
        run.worker_thread = std::thread{&LozzaxMQ::worker_thread, this, run.worker_id};
    else
        send_routed_message(workers_socket, run.worker_routing_id, "RUN");
}

void LozzaxMQ::proxy_to_worker(size_t conn_index, std::vector<zmq::message_t>& parts) {
