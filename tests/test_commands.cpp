...
TEST_CASE("deferred replies on incoming connections", "[commands][hey google]") {
    ...
    server.add_request_command("hey google", "remember", [&](Message& m) {
            ...
            if (backdoor)
                m.lozzaxmq.send(backdoor, "backdoor.data", m.data[0]);
    });
    ...
    std::vector<std::unique_ptr<LozzaxMQ>> clients;
    ...
    for (int i = 0; i < 5; i++) {
        clients.push_back(std::make_unique<LozzaxMQ>(
