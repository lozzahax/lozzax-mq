...
TEST_CASE("connections with curve authentication", "[curve][connect]") {
    ...
    LozzaxMQ client{get_logger("C» "), LogLevel::trace};
...
TEST_CASE("plain-text connections", "[plaintext][connect]") {
    std::string listen = "tcp://127.0.0.1:4455";
    LozzaxMQ server{get_logger("S» "), LogLevel::trace};
...
TEST_CASE("unique connection IDs", "[connect][id]") {
    ...
    LozzaxMQ client1{get_logger("C1» "), LogLevel::trace};
    LozzaxMQ client2{get_logger("C2» "), LogLevel::trace};
    client1.start();
    client2.start();
...
TEST_CASE("SN disconnections", "[connect][disconnect]") {
    std::vector<std::unique_ptr<LozzaxMQ>> lmq;
    ...
    for (int i = 0; i < pubkey.size(); i++) {
        lmq.push_back(std::make_unique<LozzaxMQ>(
...
TEST_CASE("SN single worker test", "[connect][worker]") {
    ...
    LozzaxMQ client{get_logger("B» "), LogLevel::trace};
    client.start();
